...
Data are at the core of scientific inquiry.  Yet, except for the very smallest of datasets, it is not feasible to print out an entire dataset and some condensation is neccesary.  Even when it is possible to print a whole dataset, it is still important to condense the data in ways that help you to communicate a story to your audience.  We summarize our data using effective visuals and **summary statistics** that describe important characteristics of the data.  The most important summaries of a variable usually describe its **location** ("middle") and **spread** (variability).
...
Statistics is about modeling randomness in the population and the sample.  
Analogues: 
 - pdf : histogram
...
The most familiar way to summarize a dataset is with the *sample mean*.  The sample mean is calculated as the sum of the individual observations $y_i$, divided by the number of observations *n*, and is denoted by $\bar{y}$:
